...
function menuItem(item, level, parent, group) {
  ...
  if (parent) {
    if (level == 0) var childOfRoot = 'child-of-ROOT ';
    else var childOfRoot = '';
  ...
  if (group)
    var expandButton =
      '<span id="menu-' + item.pathFormatted + '" style="margin-left: ' + level * 10 + 'px;" class="expand-button">&nbsp;</span>';
  else
    var expandButton =
      '<span id="menu-' + item.pathFormatted + '" style="margin-left: ' + level * 10 + 'px;" class="expand-button hidden">&nbsp;</span>';
...
function menuItemsForGroup(group, level, parent) {
  ...
  if (level > 0) items += menuItem(group, level - 1, parent, true);

  $.each(group.contents, function (contentName, content) {
    if (content.type == 'GROUP') items += menuItemsForGroup(content, level + 1, group.pathFormatted);
    else if (content.type == 'REQUEST') items += menuItem(content, level, group.pathFormatted);
  });
...
function setGlobalMenu() {
  $('.nav ul').append(
    '<li><div class="item"><a href="#active_users">Active Users</a></div></li> \
