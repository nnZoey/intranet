...
(function ($) {
  $.fn.expandable = function () {
    ...
    this.find('.expand-button:not([class*=hidden])')
      .addClass('collapse')
      .on('click', function () {
        var $this = $(this);

        if ($this.hasClass('expand')) $this.expand(scope);
        else $this.collapse(scope);
      });
  ...
  $.fn.sortTable = function (col, desc) {
    function getValue(line) {
      ...
      if (cell.hasClass('value')) var value = cell.text();
      else var value = cell.find('.value').text();
    ...
    function sortLines(lines, group) {
      ...
      $.each(sortedLines, function (i, line) {
        result.push(line);
        if (notErrorTable) result = result.concat(sortLines(lines, $(line).attr('id')));
      });
